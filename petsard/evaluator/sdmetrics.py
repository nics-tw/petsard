...
from petsard.evaluator.evaluator_base import BaseEvaluator, EvaluatorScoreConfig
from petsard.exceptions import UnsupportedMethodError
from petsard.util import safe_round


class SDMetricsSingleTableMap:
    """
...
@dataclass
class SDMetricsSingleTableScoreConfig(EvaluatorScoreConfig):
    """
    Configuration for the scoring result of sdmetrics single-table evaluator.

    Attributes:
        _logger (logging.Logger): The logger object.
        {xxx}_scores (pd.DataFrame, optional):
            - mulitple scores been defined in EvaluatorScoreConfig
    """
...
class SDMetricsSingleTable(BaseEvaluator):
    ...
    SDMETRICS_SINGLETABLE_MAP: dict[int, BaseReport] = {
    ...
    SDMETRICS_SCORES_GRANULARITY_PROPERTIES_MAP: dict[tuple[str, str], str] = {
    ...
    def __init__(self, config: dict):
        ...
        try:
            ...
            evaluator_class: Any = self.SDMETRICS_SINGLETABLE_MAP[method_code]
        except KeyError:
            error_msg: str = (
