...
class EvaluatorInputConfig(BaseConfig):
    ...
    def verify_required_inputs(self, required_input_keys: str | list[str]) -> None:
        ...
        if "Undefined" in required_input_keys:
            error_msg: str = "The required inputs are not defined."
        ...
        if len(required_input_keys) > 1:
            if "ori" not in required_input_keys:
                error_msg = (
                    "There's mulitple keys in input, but no 'ori' data for aligning dtypes. "
                ...
                metadata.build_metadata(data=self.ori)
                other_keys: list[str] = [
                    key for key in required_input_keys if key != "ori"
                ]
    ...
    def __post_init__(self):
        ...
        for granularity in self.available_scores_granularity:
            try:
                ...
                warning_msg: str = f"Non-default granularity '{granularity}' is used."
