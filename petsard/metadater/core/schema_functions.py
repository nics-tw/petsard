...
def apply_schema_transformations(
    ...
    for field_metadata in fields_to_process:
        try:
            ...
            if field_metadata.target_dtype and field_metadata.target_dtype != str(result[field_metadata.name].dtype):
                if field_metadata.target_dtype == "category":
                    result[field_metadata.name] = result[field_metadata.name].astype("category")
                elif "datetime" in field_metadata.target_dtype:
                    result[field_metadata.name] = pd.to_datetime(result[field_metadata.name], errors="coerce")
                else:
                    result[field_metadata.name] = result[field_metadata.name].astype(field_metadata.target_dtype)
        except Exception:
...
def compare_schemas(
    ...
    for field_name in common_fields:
        ...
        if field1.logical_type != field2.logical_type:
            differences.append(f"logical_type: {field1.logical_type} vs {field2.logical_type}")
    ...
    return {
        "schemas_identical": len(field_differences) == 0 and len(only_in_schema1) == 0 and len(only_in_schema2) == 0,
