    ...
    def _data_creator(n: int = 100) -> pd.DataFrame:
        data: pd.DataFrame = pd.DataFrame(
            data={
                ...
                "y_discr_str_low_card": [
                    random.choice([f"value{i}" for i in range(3)]) for _ in range(n)
                ],
                "y_discr_str_high_card": [
                    random.choice([f"value{i}" for i in range(16)]) for _ in range(n)
                ],
            }
        )
        return data
