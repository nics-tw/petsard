...
class Status:
    ...
    def get_input_from_prev(self, module: str) -> dict:
        """
        Retrieve the input of a specific module.
        ...
        """
        pre_module: str = (
            self.sequence[self.sequence.index(module) - 1] if self.sequence.index(module) >= 1
            else None
        )
        input: dict = {} # first module without input
        
        if pre_module:
            data = self.get_result(pre_module)
            if pre_module == 'Splitter':
                input['data'] = data['train']
                input['exclude_index'] =  self.get_exist_index()
            else:
                input['data'] = data
