...
class Processor:
    ...
    def _check_config_valid(self, config_to_check: dict = None) -> None:
        """
        Check whether the config contains valid preprocessors. 
        It checks the validity of column names, 
        the validity of processor types (i.e., dict keys), 
        and the validity of processor objects (i.e., dict values).
        ...
        for processor, processor_class in {
            ...
            for col in config_to_check[processor].keys():
                ...
                if not (isinstance(obj, processor_class) or
                         isinstance(obj, str) or obj is None):
                    raise ValueError(
                        f'{col} from {processor} contain(s)\
                              invalid processor object(s), \
