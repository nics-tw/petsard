...
class Encoder_Label(Encoder):
    ...
    def handle(self):
        ...
        self.dict_encoder = {}
        _digits_longest_colname = len(max(self.encoding_columns_action, key=len))
        _max_max_code = 0
        _arr_print    = []
        for _col_name in self.encoding_columns_action:
            ...
            if isinstance(_col_data.dtype ,CategoricalDtype):
                ...
                self.df_data[_col_name] = self.dict_encoder[_col_name].fit_transform(_col_data)

                _max_code = len(self.dict_encoder[_col_name].classes_)-1
                _max_max_code = max(_max_max_code ,_max_code)

                _arr_print.append([f'Preprocessor - Encoder (Label): Column {_col_name:<{_digits_longest_colname}} been labelized from 0 to '
                                 ,_max_code
                                 ,'.'])
        ...
        _arr_print = [f"{col1}{col2:>{len(str(_max_max_code))}}{col3}" for col1 ,col2 ,col3 in _arr_print]
        print('\n'.join(_arr_print))
