...
- In standard usage, includes the `method` and `method_code` from the input parameters, along with other parameters (`kwargs`). 在標準使用情況下，它包括來自輸入參數的 `method`（評測方法）、`method_code`（評測方法代號）、以及其他參數 (`kwargs`)。
