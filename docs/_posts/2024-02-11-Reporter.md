...
Since the `Reporter` is primarily designed for users to directly utilize the `Executor`, it results in a special requirement for the format of data input when using the `Reporter` independently. Please refer to [create() section](https://nics-tw.github.io/PETsARD/Reporter.html#create) in this guide for setup.

`Reporter` 模組負責產製 `PETsARD` 的成果，其中包括報告輸出、以及資料集儲存。

由於 `Reporter` 主要是為了使用者直接使用 `Executor` 而設計，變成在單獨使用 `Reporter` 時，資料的輸入格式有特別的要求，請參考本教學的 [create() 章節](https://nics-tw.github.io/PETsARD/Reporter.html#create)做設定。
...
- 而 `Reporter` 的搜尋是針對最後一組元素組。
    ...
    - For `'save_report'`, `Reporter` checks if the last pair's module name is either `Evaluator` or `Describer`, and then checks if the experiment name matches the format `{eval}_[{granularity}]`.
        - If you are certain that you want to output a single report, then the most basic `('Evaluator', 'my_eval_[global]')` would suffice.
    ...
    - 對於 `'save_report'`，`Reporter` 會檢查最後一組的模組名稱是否為 `Evaluator` 或 `Describer`，然後再檢查實驗名稱是否與 `{eval}_[{granularity}]` 這個格式匹配。
        - 如果您很清楚您就是單一個報告要輸出，那用最基本的 `('Evaluator', 'my_eval_[global]')` 即可。
...
The specific format is `({module_name}[expt_name]_)*{module_name}[expt_name]`, which means each pair of module name and experiment name is connected in the format `{module_name}[expt_name]`, and an underscore `_` is used to link between pairs.

完整實驗名稱是將完整實驗元組按照特定格式組合成一個單獨的字串，用作檔案名稱或報告中的欄位標識。

具體的格式為 `({module_name}[expt_name]_)*{module_name}[expt_name]`，也就是每組模組名稱跟實驗名稱以 `{module_name}[expt_name]` 的格式兩兩連接，而對於組跟組之間使用半形底線 `_` 連結。
