...
`eval` (`str`): refers to the `eval` in the `Reporter` parameters, representing the name given by the user for this evaluation method. 即 `Reporter` 參數中的 `eval`，代表著使用者為這個評估方式所取的名字。
