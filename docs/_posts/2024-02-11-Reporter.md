...
- The full experiment tuple comes in pairs, with an even number of elements, where each pair represents `({module_name}, {expt_name})`.
    - Module Name (`module_name`, `str`): Must be one of the following module names that can be set in `PETsARD`'s YAML, with the addition of `Processsor` for convenience. Each module name can appear in the tuple only once.

- 完整實驗元組為兩兩一組，必定是偶數個元素，其每組配對都代表著 `({模組名稱}, {實驗名稱})`。
    - 模組名稱 (`module_name`, `str`)：必須是以下 `PETsARD` 在 YAML 裡可以設定的模組名稱之一，每個模組名稱在同一個元組裡只能出現一次。
...
- 而 `Reporter` 的搜尋是針對最後一組元素組。
    ...
    - 對於 `'save_data'` 的 `source`，`Reporter` 會檢查最後一組的模組名稱或實驗名稱是否與 `source` 匹配。
        ...
        ```
            ...
            - These outputs will be combined into [full experiment names (full_expt_name)](https://nics-tw.github.io/PETsARD/Reporter.html#full_expt_name) according to the full experiment tuple, and saved to different file.
            ...
            - 針對 `source = ['Synthesizer', 'my_expt']`，共計有以下五個資料集會輸出。
                ...
                - B-1：同時配對模組跟實驗名稱
                - B-2：配對模組名稱
          　- 這些輸出會依照完整實驗元組，組合成[完整實驗名稱 (full_expt_name)](https://nics-tw.github.io/PETsARD/Reporter.html#full_expt_name)，來存成不同檔案。
